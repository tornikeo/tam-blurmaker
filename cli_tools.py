...
def get_frames_from_video(model, video_input, video_state={}):
    ...
    frames = []
    user_name = time.time()
    # If the user specified a folder, expect a list of frame files
    if not Path(video_path).suffix:
        frames = [
            cv2.cvtColor(
                cv2.imread(str(p)),
                cv2.COLOR_BGR2RGB
            ) for p in sorted(list(Path(video_input).glob('*')))
        ]
        print("WARNING: Frame rate is not extractable from images - assuming FPS==25")
        fps = 25
    else:
        operation_log = [
            ("", ""),
...
# tracking vos
def vos_tracking_video(model: TrackingAnything, video_state, interactive_state, mask_dropdown, video_output:Path=None,
    ...
    # operation error
    assert  len(np.unique(template_mask)) != 1, "There are no masks to track."
    ...
    #     video_output = "./result/track/{}".format(video_state["video_name"])
...
def generate_video_from_frames(frames, output_path, fps=30):
    ...
    torchvision.io.write_video(str(output_path), frames, fps=fps, video_codec="libx264")
    return output_path
